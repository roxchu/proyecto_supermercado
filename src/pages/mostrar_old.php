...
if (!empty($imagenes_producto)) {
    foreach ($imagenes_producto as $imagen) {
        $imagenes_js[] = "'" . addslashes($imagen['url_imagen']) . "'";
    }
}
...
<script>
    ...
    const imagenesProducto = [' . implode(',', $imagenes_js) . '];
    let imagenActual = 0;
    function navegarImagen(direccion) {
        ...
        // Controlar límites del carrusel
    ...
    function irAImagen(indice) {
        if (indice >= 0 && indice < imagenesProducto.length) {
            imagenActual = indice;
            actualizarImagenPrincipal();
        }
    }
    ...
    function actualizarImagenPrincipal() {
        ...
        if (imagenPrincipal && imagenesProducto[imagenActual]) {
            imagenPrincipal.src = imagenesProducto[imagenActual];
        }
        
        // Actualizar indicadores
        indicators.forEach((indicator, index) => {
            indicator.classList.toggle("active", index === imagenActual);
        });
    }
            ...
            <div class="producto-principal">
                <div class="producto-imagen">
                    <div class="imagen-carrusel-container">
                        ...
                        <button class="imagen-nav-btn imagen-prev" onclick="navegarImagen(-1)">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <polyline points="15,18 9,12 15,6"></polyline>
                            </svg>
                        </button>
                        <button class="imagen-nav-btn imagen-next" onclick="navegarImagen(1)">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="imagen-indicators">
                            <?php foreach ($imagenes_producto as $index => $imagen): ?>
                                <span class="imagen-indicator <?= $index === 0 ? 'active' : '' ?>" 
